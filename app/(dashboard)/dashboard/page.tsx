...
import { BookOpen, Users, FolderKanban, Mail, TrendingUp, FileText, Tag, Eye, Loader2, RefreshCw, MailOpen, ArrowRight, Clock, Send, UserCheck } from "lucide-react";
...
export default function DashboardPage() {
    ...
    const fetchAll = async () => {
        try {
            const [blogsRes, clientsRes, projectsRes, projectCatsRes, blogCatsRes, contactsRes, subscribersRes, newsletterHistoryRes] = await Promise.all([
            ...
            const [blogsData, clientsData, projectsData, projectCatsData, blogCatsData, contactsData, subscribersData, historyData] = await Promise.all([
                blogsRes.json(),
                clientsRes.json(),
                projectsRes.json(),
                projectCatsRes.json(),
            ...
            const newsletterHistory = historyData.success ? historyData.data : [];
            ...
            setStats({
                ...
                newslettersSent: newsletterHistory.length,
            });
        } catch (e) {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
                {/* Active Subscribers */}
                <Link
                    href="/dashboard/newsletter?tab=subscribers"
                    className="group flex items-center gap-4 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md"
                >
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-violet-50 group-hover:bg-violet-100 transition-colors">
                        <UserCheck className="h-5 w-5 text-violet-600" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">Newsletter</p>
                        {loading ? (
                            <div className="mt-1 h-7 w-16 animate-pulse rounded bg-slate-100" />
                        ) : (
                            <p className="text-2xl font-bold text-slate-900 leading-tight">
                                {String(stats.totalSubscribers).padStart(2, "0")}
                ...
                {/* Newsletters Sent */}
                <Link
                    href="/dashboard/newsletter?tab=history"
                    className="group flex items-center gap-4 rounded-xl border border-slate-200 bg-white px-5 py-4 shadow-sm transition-all hover:border-primary/30 hover:shadow-md"
                >
                    <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl bg-sky-50 group-hover:bg-sky-100 transition-colors">
                        <Send className="h-5 w-5 text-sky-600" />
                    </div>
                    <div className="flex-1 min-w-0">
                        <p className="text-[11px] font-semibold uppercase tracking-widest text-slate-400">Newsletters</p>
                        {loading ? (
                            <div className="mt-1 h-7 w-16 animate-pulse rounded bg-slate-100" />
                        ) : (
                            <p className="text-2xl font-bold text-slate-900 leading-tight">
                                {String(stats.newslettersSent).padStart(2, "0")}
                                <span className="ml-1.5 text-sm font-normal text-slate-400">sent so far</span>
                            </p>
                        )}
                    </div>
                    <ArrowRight className="h-4 w-4 text-slate-300 group-hover:text-primary transition-colors shrink-0" />
            ...
            {/* ── Bottom Grid — Recent Blogs + Contacts ──── */}
