...
const FOLDER_MAP: Record<string, string> = {
  ...
  "services/banners": "eventify/services/banners",
  "services/comparisons": "eventify/services/comparisons",
  "services/posters": "eventify/services/posters",
  ...
  try {
      ...
      return NextResponse.json(
        {
          success: false,
          error:
            "Invalid file type. Allowed: Images (JPEG, PNG, WebP, GIF) and Videos (MP4, WebM, OGG, MOV, AVI)",
        },
        { status: 400 },
      );
    }
    ...
    if (folderKey === "blogs" && isVideo) {
      return NextResponse.json(
        {
          success: false,
          error: "Video uploads are not allowed for blog images.",
        },
        { status: 400 },
      );
    }

    if (folderKey === "authors" && isVideo) {
      return NextResponse.json(
        { success: false, error: "Author avatars must be images, not videos." },
        { status: 400 },
      );
    }

    if (folderKey === "services/posters" && isVideo) {
      return NextResponse.json(
        {
          success: false,
          error: "Video poster must be an image, not a video.",
        },
        { status: 400 },
      );
    }
    ...
    const maxSize =
      (isVideo ? folderLimits?.video : folderLimits?.image) ?? MAX_FILE_SIZE;
    ...
    const folder =
      FOLDER_MAP[folderKey] ??
      (isVideo ? "eventify/videos" : "eventify/images");
