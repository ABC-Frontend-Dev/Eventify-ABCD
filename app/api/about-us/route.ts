    ...
    try {
        ...
        if (!aboutUs) {
            return NextResponse.json(
                { success: false, error: "About Us content not found." },
                { status: 404 }
            );
        }
...
export async function POST(request: NextRequest) {
    try {
        ...
        if (existingCount > 0) {
            return NextResponse.json(
                { success: false, error: "About Us content already exists. Use PUT to update." },
                { status: 400 }
            );
        }
        ...
        if (!body.titlePartOne || !body.description || !body.image) {
            return NextResponse.json(
                { success: false, error: "TitlePartOne, Description, and Image are required." },
                { status: 400 }
            );
        }

        if (
            body.titlePartOne.length > 50 ||
            (body.titlePartTwo && body.titlePartTwo.length > 50)
        ) {
            return NextResponse.json(
                { success: false, error: "Title parts cannot exceed 50 characters." },
                { status: 400 }
            );
        }
        ...
        if (body.cards && body.cards.length > 3) {
            return NextResponse.json(
                { success: false, error: "A maximum of 3 cards is allowed." },
                { status: 400 }
            );
        }

        // Validate each card — frontFacePartOne is required
        if (body.cards) {
            for (const card of body.cards) {
                if (!card.frontFacePartOne?.trim()) {
                    return NextResponse.json(
                        { success: false, error: "Each card must have a Front Face Part 1." },
                        { status: 400 }
                    );
                }
                if (!card.backFace?.trim()) {
                    return NextResponse.json(
                        { success: false, error: "Each card must have a Back Face." },
                        { status: 400 }
                    );
                }
            }
                ...
                cards: body.cards?.length
                    ? {
                          create: body.cards.map((card) => ({
                              frontFacePartOne: card.frontFacePartOne,
                              frontFacePartTwo: card.frontFacePartTwo || null,
        ...
        return NextResponse.json(
            { success: true, data: newAboutUs, message: "About Us created successfully." },
            { status: 201 }
        );
    } catch (error) {
        console.error("POST /api/about-us error:", error);
        return NextResponse.json(
            { success: false, error: "Failed to create About Us content." },
            { status: 500 }
        );
    }
}
...
export async function PUT(request: NextRequest) {
    try {
        ...
        if (!existingAboutUs) {
            return NextResponse.json(
                { success: false, error: "About Us content does not exist. Use POST to create." },
                { status: 404 }
            );
        }
        ...
        if (body.titlePartOne && body.titlePartOne.length > 50) {
            return NextResponse.json(
                { success: false, error: "TitlePartOne cannot exceed 50 characters." },
                { status: 400 }
            );
        }
        ...
        if (body.titlePartTwo && body.titlePartTwo.length > 50) {
            return NextResponse.json(
                { success: false, error: "TitlePartTwo cannot exceed 50 characters." },
                { status: 400 }
            );
        }
        ...
        return NextResponse.json(
            { success: true, data: updatedAboutUs, message: "About Us updated successfully." },
            { status: 200 }
        );
    } catch (error) {
...
export async function DELETE() {
    try {
        ...
        return NextResponse.json(
            { success: true, message: "About Us content deleted successfully." },
            { status: 200 }
        );
    } catch (error) {
        console.error("DELETE /api/about-us error:", error);
        return NextResponse.json(
            { success: false, error: "Failed to delete About Us content." },
            { status: 500 }
        );
    }
}