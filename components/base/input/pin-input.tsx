// components/base/input/pin-input.tsx
"use client";
...
const Slot = ({ index, className, ...props }: ComponentPropsWithRef<"div"> & { index: number }) => {
    ...
    return (
        <div
            ...
            className={cx(
                "text-xl font-helvetica-neue-roman relative flex items-center justify-center rounded-xl bg-accent text-center text-placeholder/40 shadow-xs ring-1 ring-primary transition-[box-shadow,background-color] duration-100 ease-linear ring-inset",
                styles[size].slot,
                isFocused && slot?.isActive && "ring-primary ring-2 ring-brand outline-2 outline-primary outline-offset-2 outline-brand",
                slot?.char && "text-brand-tertiary_alt ring-2 ring-primary",
                disabled && "opacity-50",
