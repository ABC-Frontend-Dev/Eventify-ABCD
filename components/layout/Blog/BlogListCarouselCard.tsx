            ...
            <div className="overflow-hidden" ref={emblaRef}>
                <div className="flex gap-1.75">
                    {CAROUSEL_DATA.map((item) => (
                        <div key={item.id} className="flex-[0_0_calc((100%-14px)/3)] min-w-0 relative">
                            ...
                            <div className="absolute w-[94%] bottom-3.5 left-1/2 -translate-x-1/2 p-3.5 bg-black/20 backdrop-blur-lg z-10">
                                <div className="border border-primary/80 bg-primary/80 rounded-[6px] p-2.5 capitalize font-product-sans-medium font-light w-fit text-white">{item.category}</div>
                                <div className="mt-1.75 text-[16px] leading-5 font-product-sans-bold font-extralight text-white">{item.title}</div>
