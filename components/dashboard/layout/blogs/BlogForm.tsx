...
export default function BlogForm({ initialData, blogId, mode }: BlogFormProps) {
  ...
  const handleChange = (
    ...
    setFormData((p) => ({
      ...p,
      [name]:
        name === "categoryId" || name === "authorId" ? parseInt(value) : value,
    }));
  };
  ...
  const handleSubmit = async (
    ...
    try {
      ...
      const res = await fetch(
        mode === "create" ? "/api/blogs" : `/api/blogs/${blogId}`,
        {
          ...
          body: JSON.stringify({
            ...
            bannerImageAlt:
              formData.bannerImageAlt || formData.metaTitle || formData.title,
          }),
        },
      );
      const result = await res.json();
      if (result.success) {
        toast.success(
          mode === "create"
            ? `Blog ${finalStatus === BlogStatus.PUBLISHED ? "published" : "saved as draft"}!`
  ...
  return (
    <div className="min-h-screen bg-slate-50/60">
        ...
        <div className="max-w-screen-2xl mx-auto px-4 sm:px-6 flex justify-between items-center gap-0 border-t border-slate-100">
          ...
          <div className="flex gap-2">
            ...
            <Button
              ...
              onClick={(e) => handleSubmit(e, BlogStatus.PUBLISHED)}
              disabled={
                loading || uploadingThumbnail || uploadingBanner || !isFormValid
              }
              className="h-7 text-xs bg-slate-900 hover:bg-slate-700 text-white"
            >
              {loading ? (
      ...
      <form
        ...
        className="max-w-screen-2xl mx-auto px-4 sm:px-6 py-6 flex flex-col xl:flex-row gap-5"
      >
        <div className="flex-1 min-w-0 space-y-5">
          {/* ── BASIC INFO ── */}
          <section
            id="basic"
            className="scroll-mt-32 bg-white border border-slate-200 rounded-xl p-5"
          >
            ...
            <div className="space-y-4">
              ...
              {/* Slug */}
              <div>
                <FieldLabel required ok={!!formData.slug.trim() && !slugError}>
                  URL Slug
                </FieldLabel>
                <div className="flex">
                  <span className="flex items-center px-2.5 text-[11px] text-slate-400 border border-r-0 border-slate-200 rounded-l-md bg-slate-50 whitespace-nowrap">
                    /blogs/
                  </span>
                  <Input
                    name="slug"
                ...
                {slugError && (
                  <p className="mt-1 text-[11px] text-red-400">{slugError}</p>
                )}
              </div>
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5">
                ...
                <div className="mt-3">
                  ...
                  <Input
                    ...
                    placeholder={
                      formData.metaTitle || formData.title || "Thumbnail image"
                    }
                    className={inp}
                  />
              ...
              <div className="bg-white border border-slate-200 rounded-xl p-5">
                ...
                <div className="mt-3">
                  ...
                  <Input
                    ...
                    placeholder={
                      formData.metaTitle || formData.title || "Banner image"
                    }
                    className={inp}
                  />
                  <p className="mt-1 text-[11px] text-slate-400">
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <Input
                  ...
                  onChange={handleChange}
                  placeholder={
                    formData.title || "Leave blank to inherit blog title"
                  }
                  className={inp}
                  maxLength={60}
                />
              ...
              <div>
                ...
                <Textarea
                  ...
                  placeholder={
                    ...
                    "Leave blank to inherit blog description"
                  }
                  rows={2}
                  className="text-sm border-slate-200 resize-none focus:border-slate-400 focus:ring-0 rounded-md placeholder:text-slate-300"
                  maxLength={160}
                ...
                {formData.keywords.length > 0 && (
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    {formData.keywords.map((kw, i) => (
                      <span
                        key={`${kw}-${i}`}
                        className="inline-flex items-center gap-1 px-2 py-0.5 bg-slate-100 text-slate-600 text-[11px] rounded-full"
                      >
                        {kw}
                        <button
                          type="button"
              ...
              <div className="space-y-1.5">
                {completion.map((item) => (
                  <div key={item.label} className="flex items-center gap-2">
                    ...
                    <span
                      className={`text-[11px] ${item.ok ? "text-slate-600" : "text-slate-400"}`}
                    >
                      {item.label}
                    </span>
                  </div>
