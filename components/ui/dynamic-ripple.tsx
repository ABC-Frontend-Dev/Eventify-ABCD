...
export function DynamicRipple({
    ...
    const createDrop = useCallback(
        ...
        [dimensions.width, dimensions.height, intensity, speed, intensityFactors, speedFactors, currentTheme.primary, currentTheme.secondary],
    );
    ...
    }, [dimensions, reactToCursor, autoAnimate, createDrop, intensity, intensityFactors]);
    ...
    return (
        <div ref={containerRef} className={cn("relative overflow-hidden", roundedStyles[rounded], className)} {...props}>
            ...
            {gradientOverlay && <div className={cn("absolute inset-0 bg-linear-to-br opacity-30 pointer-events-none", currentTheme.overlay)} />}
            {/* h-full + w-full here is the fix: without it, this wrapper has no
          explicit height, so any h-full child inside it (like your
          slide-reveal-inner) resolves "100% of an undetermined height"
          as 0 — the whole chain collapses and the canvas ends up 0x0. */}
