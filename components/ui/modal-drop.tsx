        ...
        <AnimatePresence>
            {isOpen && (
                ...
                >
                    ...
                    >
                        {title ? (
                            <div className={cn("flex justify-between p-6 pb-4", borderBottom && "border-b border-border", subtitle ? "flex-col items-start gap-1" : "items-center")}>
                                <div>
                                    <h2 className="text-xl font-semibold">{title}</h2>
                                    {subtitle && <p className="text-sm text-muted-foreground mt-1">{subtitle}</p>}
                                </div>
                                {showCloseButton && (
                                    <div className={cn("flex items-center gap-2", subtitle && "absolute top-6 right-6")}>
                                        {showEscText && <span className="hidden lg:inline px-2 py-1 text-[11px] font-medium bg-muted text-muted-foreground rounded">ESC</span>}
                                        <button className="p-1 rounded-md hover:bg-muted transition-colors" onClick={onClose} aria-label="Close modal">
                            ...
                            showCloseButton && (
                                <div className="absolute top-2.5 right-2.5 flex items-center gap-2 z-100">
                                    ...
                                    <button className="p-3 hover:bg-muted transition-colors" onClick={onClose} aria-label="Close modal">
                                        <X size={14} className="font-medium" />
                                    </button>
                                </div>
                            )
                        )}

                        <div className={cn(!disablePadding && (!title ? "p-0" : "p-0"))}>{children}</div>
                    </motion.div>
                </motion.div>
            )}
        </AnimatePresence>
