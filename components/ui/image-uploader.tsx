// app/components/ui/image-uploader.tsx
 ...
 */
export const ImageUploader = React.forwardRef<HTMLDivElement, ImageUploaderProps>(({ files, onChange, maxFiles = 5, maxSize = 4, accept = "image/*", className, ...props }, ref) => {
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
                    <AnimatePresence>
                        {previewUrls.map((url, index) => (
                            ...
                            >
                                <img src={url} alt={`Preview of ${files[index].name}`} className="object-cover w-full h-full rounded-md" />
                                <Button
                                    type="button"
                                    variant="destructive"
                                    size="icon"
